            ...
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3 ">
                ...
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                    ...
                    <li class="breadcrumb-item text-muted">
                        <a href="{{ route('kas-keluar.index') }}" class="text-muted text-hover-primary">
                            Kas Keluar </a>
                    </li>
                    <li class="breadcrumb-item">
                        <span class="bullet bg-gray-400 w-5px h-2px"></span>
                    </li>
                    <li class="breadcrumb-item text-muted">Edit Kas Keluar</li>
                </ul>
            </div>
        </div>
    </div>
            ...
            <form class="form-kas-keluar">
                <div class="row">
                    <div class="col-md-4 mb-2">
                        ...
                        <input type="text" readonly value="{{ $kasKeluar->kode }}"
                            class="form-control form-control-solid" placeholder="Kode" name="kode" />
                    </div>
                    <div class="col-md-4 mb-2">
                        ...
                        <select class="form-select form-select-solid" name="kode_akun" data-control="select2"
                            ...
                            @foreach ($akuns as $akun)
                                <option @selected($kasKeluar->kode_akun == $akun->kode) value="{{ $akun->kode }}">
                    ...
                    <div class="col-md-4 mb-2">
                        ...
                        <select class="form-select form-select-solid" name="penerima" data-control="select2"
                            ...
                            @foreach (\App\Enums\Penerima::cases() as $penerima)
                                <option @selected($kasKeluar->penerima == $penerima) value="{{ $penerima->value }}">{{ $penerima->label() }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4 mb-2 siswa"
                        style="{{ $kasKeluar->penerima == \App\Enums\Penerima::SISWA ? '' : 'display: none;' }}">
                        ...
                        <select class="form-select form-select-solid" name="siswa_id" data-control="select2"
                            ...
                            @foreach ($siswas as $siswa)
                                <option @selected($kasKeluar->siswa_id == $siswa->getKey()) value="{{ $siswa->getKey() }}">{{ $siswa->nama }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4 mb-2 pemasok"
                        style="{{ $kasKeluar->penerima == \App\Enums\Penerima::PEMASOK ? '' : 'display: none;' }}">
                        ...
                        <select class="form-select form-select-solid" name="pemasok_id" data-control="select2"
                            ...
                            @foreach ($pemasoks as $pemasok)
                                <option @selected($kasKeluar->pemasok_id == $pemasok->getKey()) value="{{ $pemasok->getKey() }}">{{ $pemasok->nama }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4 mb-2 guru"
                        style="{{ $kasKeluar->penerima == \App\Enums\Penerima::GURU ? '' : 'display: none;' }}">
                        ...
                        <select class="form-select form-select-solid" name="guru_id" data-control="select2"
                            ...
                            @foreach ($gurus as $guru)
                                <option @selected($kasKeluar->guru_id == $guru->getKey()) value="{{ $guru->getKey() }}">{{ $guru->nama }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4 mb-2">
                        ...
                        <textarea name="keterangan" class="form-control form-control-solid">{{ $kasKeluar->keterangan }}</textarea>
                    </div>
                </div>
                <hr>
                <table id="detail-kas-keluar" class="table table-bordered">
                    ...
                    <tbody data-repeater-list="detail_transaksi">
                        @forelse ($kasKeluar->jurnal?->jurnalDetails as $jurnalDetail)
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.simpan').click(function(e) {
                ...
                $.ajax({
                    type: 'POST',
                    url: `/kas-keluar/{{ $kasKeluar->kode }}/update`,
                    processData: false,
                    contentType: false,
                    data: postData,
