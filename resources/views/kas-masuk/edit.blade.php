...
@section('content')
    <div class="card">
        <div class="card-body">
            <form class="form-kas-masuk">
                <div class="row">
                    <div class="col-md-4 mb-2">
                        <label class="required fs-6 fw-semibold mb-2">Kode</label>
                        <input type="text" readonly value="{{ $kasMasuk->kode }}" class="form-control form-control-solid"
                    ...
                    <div class="col-md-4 mb-2">
                        ...
                        <select class="form-select form-select-solid" name="kode_akun" data-control="select2"
                            ...
                            @foreach ($akuns as $akun)
                                <option @selected($kasMasuk->kode_akun == $akun->kode) value="{{ $akun->kode }}">
                    ...
                    <div class="col-md-4 mb-2">
                        ...
                        <select class="form-select form-select-solid" name="dari" data-control="select2"
                            data-placeholder="Dari">
                            <option></option>
                            @foreach (\App\Enums\Dari::cases() as $dari)
                                <option @selected($kasMasuk->dari == $dari) value="{{ $dari->value }}">{{ $dari->label() }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4 mb-2 siswa"
                        style="{{ $kasMasuk->dari == \App\Enums\Dari::SISWA ? '' : 'display: none;' }}">
                        ...
                        <select class="form-select form-select-solid" name="siswa_id" data-control="select2"
                            ...
                            @foreach ($siswas as $siswa)
                                <option @selected($kasMasuk->siswa_id == $siswa->getKey()) value="{{ $siswa->getKey() }}">{{ $siswa->nama }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4 mb-2 pemasok"
                        style="{{ $kasMasuk->dari == \App\Enums\Dari::PEMASOK ? '' : 'display: none;' }}">
                        ...
                        <select class="form-select form-select-solid" name="pemasok_id" data-control="select2"
                            ...
                            @foreach ($pemasoks as $pemasok)
                                <option @selected($kasMasuk->pemasok_id == $pemasok->getKey()) value="{{ $pemasok->getKey() }}">{{ $pemasok->nama }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4 mb-2 guru"
                        style="{{ $kasMasuk->dari == \App\Enums\Dari::GURU ? '' : 'display: none;' }}">
                        ...
                        <select class="form-select form-select-solid" name="guru_id" data-control="select2"
                            ...
                            @foreach ($gurus as $guru)
                                <option @selected($kasMasuk->guru_id == $guru->getKey()) value="{{ $guru->getKey() }}">{{ $guru->nama }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4 mb-2">
                        ...
                        <textarea name="keterangan" class="form-control form-control-solid">{{ $kasMasuk->keterangan }}</textarea>
                    </div>
                </div>
                <hr>
                <table id="detail-kas-masuk" class="table table-bordered">
                    ...
                    <tbody data-repeater-list="detail_transaksi">
                        @forelse ($kasMasuk->jurnal?->jurnalDetails as $jurnalDetail)
                            <tr data-repeater-item>
                                <td style="width: 22%;">
                                    <div class="col-md-12 akun">
                                        <select class="form-select form-select-solid" name="kode_akun"
                                            data-control="select2" data-kt-repeater="akun" data-placeholder="Akun"
                                            data-dropdown-parent=".akun">
                                            <option></option>
                                            @foreach ($akuns as $akun)
                                                <option @selected($jurnalDetail->kode_akun == $akun->kode) value="{{ $akun->kode }}">
                                                    {{ $akun->nama }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div class="col-md-12 klasifikasi">
                                        <select class="form-select form-select-solid" name="klasifikasi_id"
                                            ...
                                            @foreach ($klasifikasis as $klasifikasi)
                                                <option @selected($jurnalDetail->klasifikasi_id == $klasifikasi->id) value="{{ $klasifikasi->id }}">
                                                    {{ $klasifikasi->nama }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <td style="width: 22%;">
                                    <div class="col-md-12">
                                        <input class="form-control form-control-solid uang debet" name="debet"
                                            ...
                                            value="{{ number_format($jurnalDetail->debet, 0, ',', '.') }}" />
                                    </div>
                                </td>
                                <td style="width: 22%;">
                                    <div class="col-md-12">
                                        <input class="form-control form-control-solid uang kredit" name="kredit"
                                            ...
                                            value="{{ number_format($jurnalDetail->kredit, 0, ',', '.') }}" />
                                    </div>
                                </td>
                                <td style="width: 10%;">
        ...
        $(document).ready(function() {
            ...
            $('select[name="dari"]').change(function(e) {
                ...
                if (value == "{{ \App\Enums\Dari::SISWA }}") {
                    ...
                    $('.siswa').hide();
                    $('.guru').hide();
                } else if (value == "{{ \App\Enums\Dari::GURU }}") {
                    $('.guru').show();
                    $('.pemasok').hide();
                    $('.siswa').hide();
                } else {
            ...
            $('.simpan').click(function(e) {
                ...
                var postData = new FormData($(".form-kas-masuk")[0]);
                $.ajax({
                    type: 'POST',
                    url: `/kas-masuk/{{ $kasMasuk->kode }}/update`,
                    processData: false,
                    contentType: false,
                    data: postData,
