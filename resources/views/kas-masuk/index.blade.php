...
@section('content')
    <div class="card">
            ...
            <div class="card-toolbar">
                <div class="d-flex justify-content-end" data-kt-customer-table-toolbar="base">
                    ...
                    <button type="button" class="btn btn-primary ps-4" data-bs-toggle="modal"
                        data-bs-target="#create-kas-masuk">
                        <i class="ki-duotone ki-plus fs-2"></i>Tambah Kas Masuk
                    </button>
                </div>
        ...
        <div class="card-body pt-0">
            <table class="table align-middle table-row-dashed fs-6 gy-5" id="kas_masuk_table">
                <thead>
                    <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
                        <th class="w-10px pe-2">
                            <div class="form-check form-check-sm form-check-custom form-check-solid me-3">
                                <input class="form-check-input" type="checkbox" data-kt-check="true"
                                    data-kt-check-target="#kas_masuk_table .form-check-input" value="1" />
                            </div>
                        </th>
                        <th>No Kas Masuk</th>
                        <th>Tanggal</th>
                        ...
                        <tr>
                            <td>
                                <div class="form-check form-check-sm form-check-custom form-check-solid">
                                    <input class="form-check-input" type="checkbox" value="{{ $kasMasuk->getKey() }}" />
                                </div>
                            </td>
                            <td>{{ $kasMasuk->no }}</td>
                            <td>{{ $kasMasuk->tanggal }}</td>
                            <td>{{ transaksiDari($kasMasuk) }}</td>
                            <td>{{ $kasMasuk->keterangan }}</td>
                            <td>
                                <div class="d-flex align-items-center gap-2">
                                    <button type="button" data-kas-masuk="{{ $kasMasuk->getKey() }}"
...
@push('modal')
    <div class="modal fade" id="create-kas-masuk" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-xl">
            <div class="modal-content">
                <form class="form" action="#" id="create-kas-masuk_form">
                    <div class="modal-header" id="create-kas-masuk_header">
                        <h2 class="fw-bold">Tambah Kas Masuk</h2>
                        <div id="create-kas-masuk_close" class="btn btn-icon btn-sm btn-active-icon-primary">
                            <i class="ki-duotone ki-cross fs-1"><span class="path1"></span><span class="path2"></span></i>
                        </div>
                    </div>
                    <div class="modal-body py-10 px-lg-17 row">
                        <div class="col-md-6 mb-7">
                            <label class="required fs-6 fw-semibold mb-2">No Kas Masuk</label>
                            <input type="text" class="form-control form-control-solid" readonly
                                value="{{ $no }}" placeholder="No Kas Masuk" name="no" />
                        </div>
                        <div class="col-md-6 mb-7">
                        ...
                        <div class="col-md-6 mb-7">
                            ...
                            <select class="form-select form-select-solid" name="dari" data-control="select2"
                                data-placeholder="Dari" data-dropdown-parent="#create-kas-masuk">
                                <option></option>
                        ...
                        <div class="col-md-6 mb-7 siswa" style="display: none;">
                            ...
                            <select class="form-select form-select-solid" name="siswa" data-control="select2"
                                data-placeholder="Siswa" data-dropdown-parent="#create-kas-masuk">
                                <option></option>
                        ...
                        <div class="col-md-6 mb-7 pemasok" style="display: none;">
                            ...
                            <select class="form-select form-select-solid" name="pemasok" data-control="select2"
                                data-placeholder="Pemasok" data-dropdown-parent="#create-kas-masuk">
                                <option></option>
                        ...
                        <hr>
                        <table id="detail-kas-masuk" class="table table-bordered">
                            <thead>
                                <tr>
                                    <th>Akun</th>
                                    <th>Klasifikasi</th>
                    ...
                    <div class="modal-footer flex-center">
                        <button type="reset" id="create-kas-masuk_cancel" class="btn btn-light me-3">
                            Discard
                        </button>
                        <button type="button" data-akun="" id="create-kas-masuk_submit" class="btn btn-primary">
                            <span class="indicator-label">
                                Submit
                            </span>
                            <span class="indicator-progress">
...
@push('js')
    ...
    <script src="{{ asset('assets/js/pages/kas-masuk/index.js') }}"></script>
    {{-- <script src="{{ asset('assets/js/pages/kas-masuk/create.js') }}"></script> --}}
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#create-kas-masuk_submit').click(function(e) {
                e.preventDefault();
                var target = this;
                $(this).attr('data-kt-indicator', 'on');
                var postData = new FormData($("#create-kas-masuk_form")[0]);
                $.ajax({
                    type: 'POST',
                    url: `/kas-masuk/store`,
                    processData: false,
            ...
            $('#detail-kas-masuk').repeater({
                ...
                show: function() {
                    $(this).slideDown();
                    $('#detail-kas-masuk .select2-container').remove();
                    $('#detail-kas-masuk [data-kt-repeater="akun"]').each(function(index, element) {
                        $(element).attr('id', `akun${index}`);
                        $(element).select2({
                            width: '100%',
                        });
                    });
                    $('#detail-kas-masuk [data-kt-repeater="klasifikasi"]').each(function(index,
                        element) {
